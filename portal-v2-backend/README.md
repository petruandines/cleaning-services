...
- `wrangler.jsonc` conține UUID-ul D1 și adresa Workerului furnizate de proprietar; înainte de prima migrație remote confirmă în Cloudflare că UUID-ul aparține bazei `petru-ines-portal-eu` cu jurisdicție EU. Un `wrangler deploy --dry-run` validează numai construcția locală. Nu executa deploy până când migrațiile, secretul și rate limiting sunt verificate.
