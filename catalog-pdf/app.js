...
function recalcFitScale() {
  ...
  const pad = isSmallScreen() ? 0 : 12;
  state.fitScale = Math.max(0.01, Math.min(
    (stageWidth - pad * 2) / state.baseWidth,
    (stageHeight - pad * 2) / state.baseHeight,
  ));
}
...
function clampPan() {
  ...
  const maxX = width > stageWidth ? (width - stageWidth) / 2 + overscroll : 0;
  const maxY = height > stageHeight ? (height - stageHeight) / 2 + overscroll : 0;
...
function zoomFromCenter(factor) {
  ...
  setZoomAt(rect.left + rect.width / 2, rect.top + rect.height / 2, state.zoom * factor);
}
...
async function renderCurrentPage({ transitionDirection = 0 } = {}) {
  ...
  await Promise.all([
    ...
    renderHighlights(page, cssViewport, token),
  ]);
...
function wireEvents() {
  ...
  els.shareBtn.addEventListener('click', async () => {
    ...
    const shareData = { title: document.title, text: `Pagina ${state.currentPage} din catalog`, url: url.href };
    try {
      if (navigator.share) {
