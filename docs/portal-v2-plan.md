...
### Stare la 25 septembrie 2026 — primul administrator creat și verificat în D1

- Proprietarul a salvat parola administratorului în GitHub secretul temporar `PORTAL_ADMIN_INITIAL_PASSWORD`, a păstrat-o privat și a rulat **o singură dată** `create` în [rularea #2](https://github.com/petruandines/cleaning-services/actions/runs/36110568017) pe `main`. Jobul `bootstrap` este **Success**; `inspect` a fost sărit, pasul `create` a trecut. Logul confirmă `OPERATION: create`, adresa `petruandines@gmail.com`, confirmarea exactă cu UUID-ul D1, verificările EU/schema/migrații/tabele goale, apoi `One administrator and one credential account verified; no client records created.`
- Contul admin există acum în baza D1 și are credential hashuit; **nu relansa `create`**. Nu există încă probă de autentificare reală/TOTP. Portalul public GitHub Pages rămâne Appwrite.
- Pasul imediat: revocă tokenul temporar Cloudflare pentru `PORTAL_D1_ADMIN_TOKEN` și șterge din GitHub secretele `PORTAL_D1_ADMIN_TOKEN` și `PORTAL_ADMIN_INITIAL_PASSWORD`; păstrează parola în locul privat și secretul permanent `PORTAL_WORKER_AUTH_SECRET`. Urmează verificarea loginului/TOTP pe telefon și apoi date fictive, backup/restore remote distinct și QA înainte de comutare.


### Stare la 25 septembrie 2026 — credentiale temporare retrase, previzualizare live

- Proprietarul a confirmat revocarea tokenului Cloudflare folosit la bootstrap și ștergerea din GitHub a secretelor `PORTAL_D1_ADMIN_TOKEN` și `PORTAL_ADMIN_INITIAL_PASSWORD`. Cheia permanentă `PORTAL_WORKER_AUTH_SECRET` și parola administratorului salvată privat rămân necesare. Nu repeta bootstrapul.
- Pentru a putea testa fluxul real de autentificare din telefon, cele patru fișiere deja revizuite din `portal-v2-frontend/` (HTML, CSS, config, JS) au fost copiate **într-un singur commit** `5b634e87b13c3923725355cb94f5164f382f8506` pe `main`. Aceasta creează exclusiv previzualizarea `https://petruandines.github.io/cleaning-services/portal-v2-frontend/`; nu atinge directorul `/portal/`, care continuă să folosească Appwrite. Pagina de previzualizare s-a încărcat public în browser cu butonul `Intră în portal ↗`.
- Pas imediat pe dispozitivul proprietarului: deschide previzualizarea, apasă `Intră în portal ↗`, autentifică-te cu `petruandines@gmail.com` și parola privată; oprește-te și raportează ce ecran apare. Loginul Worker solicită deschiderea din previzualizare cu `state` aleator. Dacă apare activarea TOTP, nu trimite în chat URI-ul, codul QR ori codurile de recuperare și nu captura ecranul cu ele. Testul real de autentificare, TOTP, backup/restore remote și QA mobil/Safari rămân de finalizat înaintea comutării publice.


### Stare la 25 septembrie 2026 — login parolă reușit, TOTP încă neactivat

- Proprietarul a deschis previzualizarea `https://petruandines.github.io/cleaning-services/portal-v2-frontend/`, a pornit fereastra Workerului și a introdus adresa/parola inițială. Captura arată pasul `Activează verificarea în doi pași` pe originul Workerului (formular `Confirmă parola` + `Generează cheia TOTP`). Acest ecran apare după autentificare și răspunsul `/api/me`, deci loginul cu parolă este confirmat în browser. Nu confirmăm încă generarea cheii, salvarea codurilor de recuperare ori activarea TOTP; nu s-a intrat în interfața staff.
- Pasul imediat: confirmă aceeași parolă în fereastra Worker și apasă `Generează cheia TOTP`; apoi proprietarul trebuie să salveze separat codurile de recuperare și să configureze aplicația TOTP, fără a transmite în chat/capturi URI-ul `otpauth://`, cheia secretă, codurile de recuperare sau codurile de șase cifre. Continuați pas cu pas până la intrarea în interfață. Portalul public `/portal/` rămâne Appwrite.


### Stare la 25 septembrie 2026 — autentificare verificată și îmbunătățiri cerute după primul acces

- Proprietarul s-a autentificat cu contul de administrator, a configurat TOTP și a intrat în portalul de previzualizare `/cleaning-services/portal-v2-frontend/`. A observat că autentificarea este prea anevoioasă pentru clienți și că reîncărcarea paginii îl deconecta.
- Cu preferința explicită „Automat în fila curentă”, front-endul păstrează tokenul temporar în `sessionStorage` pentru maximum 8 ore, îl revalidează prin `/api/me` la refresh și îl șterge la ieșire/expirare. A fost publicat pe `main` în commitul `e0c10239abb6eb779edc7d0c33f6f302e0d50f91`, GitHub Pages #36112684746 verde. **Proprietarul încă nu a confirmat refresh în aceeași filă după o nouă autentificare**, deci testul real pe telefon rămâne de făcut.
- Captura următoare a arătat titlul greșit „Programăr” la programări, generat de scurtarea mecanică a pluralului. În ramura draft s-a pus singular explicit „Programare”; titlurile programărilor folosesc numele clientului, iar cardurile includ numele și adresa locației. Backendul `GET /api/appointments` aduce aceste date prin join restrâns la client; `GET /api/messages` aduce numele expeditorului și clientului. Interfața folosește carduri responsive pentru toate secțiunile și bule de conversație pentru mesaje, cu text redat sigur ca text simplu.
- Verificat local: 24/24 teste backend, inclusiv izolarea între clienți și joinurile noi; 2/2 teste sesiune frontend și verificarea sintaxei JS. Schimbările API/UI sunt salvate **numai pe ramura draft** `codex/portal-v2-plan`, nu sunt încă instalate în Worker ori în previzualizarea `main`. Ultimul commit de cod draft: `ddca8f9b74a60ec3c20e4e53e36a37560951671e`. Portalul public `/cleaning-services/portal/` rămâne pe Appwrite.
- Cerințe de produs primite de la proprietar, de implementat înainte de lansare: personalul să poată edita sau șterge ulterior fiecare programare, lucrare, plată, mesaj, locație și client; interfețe mai frumoase pentru toate filele; mesaje intuitive ca într-un chat pe mobil; câmpuri **opționale** pentru numele, telefonul și e-mailul persoanei de contact la fiecare locație. Pentru contact e necesară o migrație D1 **nouă**, fără a schimba cele trei migrații deja aplicate. Pentru ștergeri e necesară decizia privind istoricul plăților, mesajelor și conturilor asociate, cu autorizații și audit. **Nicio ștergere sau migrare nouă nu a rulat.**
- Reluare economică: citește acest checkpoint, verifică modificările draft și nu repeta cercetarea Cloudflare. Mai întâi testează cu proprietarul refresh după login în fila existentă. Apoi implementează editarea/ștergerea cu protecție pentru istoricul financiar și relații, plus migrația pentru persoana de contact, teste locale și operație de migrare remote separată, verificată. Publică Workerul și front-endul în previzualizare împreună, după verificarea tokenului temporar necesar; nu cere nimănui să transmită tokenuri în chat. Testează pe telefon, backup/restaurare și abia apoi hotărăște trecerea portalului public.


### Stare la 25 septembrie 2026 — refresh confirmat și contact la locație pregătit

- Proprietarul a testat după autentificare o reîncărcare în aceeași filă și a confirmat „da, rămân”: sesiunea persistă efectiv în browserul său. Nu este necesar să repetăm această probă fără o regresie concretă.
- Pe ramura draft, fișierul **nou** `docs/portal-v2/0004_location_contact.sql` adaugă coloanele opționale `contact_name`, `contact_phone`, `contact_email` la locații; valorile existente rămân NULL. Frontendul oferă câmpuri opționale, iar API validează lungimile și formatul e-mailului și expune datele numai în aria de acces a clientului. Migrațiile inițiale 0001–0003 și hashurile lor rămân neschimbate.
- Verificat local cu SQL 0004 aplicat **numai într-o bază SQLite temporară de test**: 25/25 teste backend și 2/2 teste frontend trec, inclusiv adăugare locație fără contact, cu contact și respingerea e-mailului invalid și izolarea între clienți. Ultimul commit de cod draft `b234a7638c326d2abea47a81df8a40120ea0ccd9`.
- **Nicio migrare remote nu a fost rulată.** Workerul publicat și interfața de previzualizare pe `main` nu conțin noile câmpuri/carduri. Înainte de publicarea Workerului din draft, aplică separat și verifică migrarea D1 0004, cu identitate și jurisdicție verificate, backup și operație explicită; codul nou citește coloanele și ar eșua dacă Workerul s-ar publica înainte de migrare. Fluxul inițial `portal-d1-migrate.yml` este pentru o bază goală și **nu trebuie relansat**. Tokenul D1 Edit anterior a fost revocat.
- Cerința rămasă majoră: editare și ștergere pentru programări, lucrări, plăți, mesaje, locații și clienți, cu audit, permisiuni și reguli clare pentru legături/istoricul plăților. Continuă de la acestea și pregătește o migrare și un workflow controlat de upgrade pentru 0004; apoi publică împreună API și interfață în previzualizare. Portalul live rămâne Appwrite.


### Stare la 25 septembrie 2026 — editare și arhivare în draft, fără publicare

- Proprietarul a confirmat testul de refresh și a cerut continuarea implementării. În ramura `codex/portal-v2-plan` există `0005_soft_delete.sql` pentru cele șase tipuri (clienți, locații, programări, lucrări, plăți, mesaje). Personalul cu TOTP poate modifica câmpuri validate și arhiva înregistrări din cardurile interfeței; clientul nu poate folosi PATCH/DELETE.
- Arhivarea setează `deleted_at` și ascunde rândul din liste, păstrând istoricul în D1 și un eveniment de audit atomic. Ștergerea este blocată cu HTTP 409 cât timp există înregistrări dependente active (client → locații/programări/lucrări/plăți/mesaje; locație → programări; programare → lucrări; lucrare → plăți). Un client arhivat pierde legătura conturilor sale, iar clientul inactiv nu mai primește acces API. Nu se modifică proprietarul unei înregistrări prin PATCH, iar referințele către alt client sunt refuzate.
- Scriptul de migrare **inițială** din draft refuză acum fișierele SQL suplimentare, pentru a nu rula accidental 0004/0005 la `apply`; workflow-ul inițial pe `main` rămâne fixat la commitul revizuit anterior și nu trebuie relansat. Pentru upgrade pregătește un workflow **nou**, cu verificarea UUID-ului, numelui și jurisdicției EU, a migrațiilor existente, hashurilor 0004/0005, operații distincte inspect/apply și confirmare exactă. Fă backup înainte de migrare.
- Testat local cu migrațiile 0001–0005 în baze de test: **29/29 teste backend**, inclusiv control acces, editare, dependențe, audit atomic, retragerea accesului, și **2/2 teste sesiune frontend**; `node --check app.js` a trecut. Codul și migrațiile sunt salvate în ramura draft, ultima actualizare interfață `ec8cd3ad009fffbb3038b6906b8a04a8dc4d80ba`; backendul și previzualizarea de pe `main` **nu sunt actualizate**. D1 remote are încă doar 0001–0003, deci NU publica Workerul din draft înainte de 0004/0005.
- Etapele rămase înainte de lansare: revizuire suplimentară a modificărilor și migrărilor, workflow-ul nou de upgrade cu testele lui, backup verificat, aplicație manuală în Cloudflare cu un token temporar nou păstrat numai în secrete GitHub, publicare coordonată Worker + preview, QA pe telefon cu client fictiv și verificarea restaurării. Live `/cleaning-services/portal/` rămâne Appwrite. Nu cere din nou dovada refreshului și nu repeta testele anterioare fără un risc concret.


### Stare la 25 septembrie 2026 — workflow separat de upgrade pregătit, nerulat

- În ramura draft există `.github/workflows/portal-d1-upgrade.yml`, cu `inspect` / `apply`, checkout fix la commitul revizuit `7387062c47ad5a11e79692de1d507c1175b4b006`, repository/branch `main` și concurență controlată. Folosește un secret **nou** `PORTAL_D1_UPGRADE_TOKEN`, care **nu există** acum, pentru o permisiune temporară D1 Edit limitată la contul corect. Nu a fost copiat pe `main`, deci nu apare încă în lista de workflow-uri manuale acolo.
- `portal-v2-backend/scripts/upgrade-phone.mjs` refuză orice fișier SQL în plus sau cu hash diferit, UUID/nume/jurisdicție nepotrivite, istoric de migrare diferit de exact 0001–0003 sau coloane parțial aplicate. Înainte de scriere cere `APPLY PORTAL UPGRADE 6816004b-dc95-48c9-be52-9bd4131d157e` și confirmarea separată `BACKUP VERIFIED 6816004b-dc95-48c9-be52-9bd4131d157e`. După aplicație verifică exact 0001–0005 și toate cele 6 coloane de arhivare + 3 coloane contact. **Confirmarea textuală nu dovedește existența unui backup; verificarea efectivă trebuie realizată separat înainte de apply.**
- Testat: **33/33 backend**, 2/2 sesiune frontend, sintaxă JS și YAML; testul nou aplică toate cele 5 SQL-uri într-o bază SQLite izolată cu o înregistrare preexistentă și verifică păstrarea ei și schema rezultată. Niciun token nu a fost creat, nu s-a rulat `inspect` ori `apply` pe Cloudflare și nu s-a publicat un Worker sau o interfață nouă. Workflow-ul inițial 0001–0003 nu trebuie relansat.
- Pași următori pentru lansare: creează și **verifică efectiv** un backup D1 remote, pregătește workflow-ul de upgrade pe `main` după review, folosește token D1 Edit temporar introdus direct în GitHub Secrets, rulează doar `inspect`, examinează rezultatul și numai apoi `apply` cu confirmările exacte. Revocă tokenul după migrare. Pregătește separat un workflow de actualizare a Workerului existent, cu token Workers Scripts Edit temporar și secretul permanent al Workerului; publică în previzualizare și testează date fictive înainte de a comuta `/portal/` public. Nu repeta testul deja confirmat de refresh în aceeași filă.
