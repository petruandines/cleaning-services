(() => {
  ...
  async function runOCR(sourceCanvas, scale) {
    ...
    try {
      ...
      }).filter(w => w.text && w.confidence>=minConfidence && ((w.bbox.y1-w.bbox.y0)/sourceCanvas.height)<=maxRatio && (w.bbox.x1-w.bbox.x0)>2 && (w.bbox.y1-w.bbox.y0)>2);
    } finally {
  ...
  function removeTinyVectorPaths(svg) {
    ...
    svg.querySelectorAll('#base-trace path').forEach(p=>{
  ...
  function updateStats(){
    ...
    els.statPaths.textContent=paths.length.toLocaleString('ro-RO'); els.statAnchors.textContent=anchors.toLocaleString('ro-RO'); els.statTexts.textContent=texts.length; els.statShapes.textContent=shapes.length;
    const density=paths.length?anchors/paths.length:0; const px=state.workWidth*state.workHeight;
    let score=100-Math.min(55,density*1.5)-Math.min(30,(paths/Math.max(1,px/10000))*2.2);
